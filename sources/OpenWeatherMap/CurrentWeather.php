...
use AJUR\OpenWeatherMap\Util\Temperature;
use AJUR\OpenWeatherMap\Util\Unit;
use AJUR\OpenWeatherMap\Util\Weather;
use AJUR\OpenWeatherMap\Util\Wind;
...
class CurrentWeather
{
    ...
    public function __construct($data, $units)
    {
        ...
        if ($data instanceof \SimpleXMLElement) {
            $this->city = new City( (int)$data->city[ 'id' ], $data->city['name'], $data->city->coord['lat'], $data->city->coord['lon'], $data->city->country);
            ...
            $this->weather = new Weather( (int)$data->weather[ 'number' ], $data->weather['value'], $data->weather['icon']);
            ...
            $rain = property_exists($data, 'rain') && $data->rain !== null ? (array) $data->rain : [];
